...
const AuthenticatedHeader: React.FC = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 flex items-center justify-between ${isDarkMode ? "bg-gray-900 border-gray-700" : "bg-gradient-to-r from-[var(--primary)] to-[var(--primary-dark)] border-gray-700"} border-b px-4 md:px-10 py-3 w-full transition-colors duration-300 z-40 shadow-md`}
    >
      <div className={`flex items-center gap-4 ${isDarkMode ? "text-white" : "text-white"}`}>
      ...
      <div className="flex items-center gap-4 md:gap-8">
        <nav
          className={`hidden md:flex mr-40 gap-20 md:gap-35 text-sm font-medium ${isDarkMode ? "text-gray-200" : "text-white"}`}
        >
          <Link
            to="/dashboard"
            className={`transition-colors duration-200 ${isDarkMode ? "hover:text-blue-400" : "hover:text-[var(--primary)] hover:scale-105"}`}
          >
            Dashboard
          </Link>
          <Link
