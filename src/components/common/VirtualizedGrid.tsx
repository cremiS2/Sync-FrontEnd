...
function ResponsiveGrid<T>({
  ...
  const columnCount = useMemo(() => {
    ...
    if (containerWidth < 640) cols = 2; // Mobile - 2 colunas
    else if (containerWidth < 768) cols = 2; // Mobile landscape - 2 colunas
  ...
  const gridClass = useMemo(() => {
    const baseClass = 'grid gap-2 sm:gap-4 md:gap-6 lg:gap-8';
    // Mobile: 2 colunas, Tablet: 2, Desktop: 3-4-5
    return `${baseClass} grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5`;
  }, []);
