...
const ItemCard: React.FC<ItemCardProps> = ({
  ...
  return (
    <Card className={`shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden h-full ${className}`}>
      <CardContent className="p-6 h-full flex flex-col">
        ...
        <div className="flex items-start justify-between mb-4 gap-2">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-lg font-bold text-[var(--text)] truncate" title={title}>{title}</h3>
              {subtitle && (
                <p className="text-sm text-[var(--muted)] truncate" title={subtitle}>{subtitle}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-4 flex-grow">
          {fields.map((field, index) => (
            <div key={index} className="flex justify-between items-center gap-2 min-w-0">
              <span className="text-sm font-medium text-[var(--primary)] flex-shrink-0">
                {field.label}:
              </span>
              <div className="truncate">{renderFieldValue(field)}</div>
            </div>
          ))}
        </div>
