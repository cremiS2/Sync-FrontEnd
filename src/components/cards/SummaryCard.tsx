...
const SummaryCard: React.FC<SummaryCardProps> = ({
  ...
  return (
    <Card className={`shadow-lg border-0 ${colors.card} hover:shadow-xl transition-all duration-300 transform hover:scale-105 overflow-hidden h-full ${className}`}>
      <CardContent className="p-6 h-full">
        <div className="flex items-center justify-between gap-3 h-full">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-2">
              <div className={`w-3 h-3 ${colors.indicator} rounded-full flex-shrink-0`}></div>
              <p className={`text-sm font-medium ${colors.title} truncate`} title={title}>{title}</p>
            </div>
            <p className={`text-3xl font-bold ${colors.value} mb-1 truncate`}>{value}</p>
            {trend && (
            ...
            {subtitle && (
              <p className={`text-xs ${colors.subtitle} flex items-center gap-1 truncate`} title={subtitle}>
                {subtitle}
              </p>
            )}
          </div>
