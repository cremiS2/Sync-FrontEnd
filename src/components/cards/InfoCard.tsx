...
const InfoCard: React.FC<InfoCardProps> = ({
  ...
  return (
    <Card 
      className={`shadow-lg border-0 ${colors.card} hover:shadow-xl transition-all duration-300 transform hover:scale-105 overflow-hidden h-full ${className}`}
      onClick={onClick}
      ...
      <CardContent className="p-6 h-full">
        <div className="flex items-center justify-between gap-3 h-full">
          <div className="min-w-0 flex-1">
            ...
            {subtitle && (
              <p className={`text-xs ${colors.subtitle} truncate`} title={subtitle}>
                {subtitle}
              </p>
            )}
          </div>
