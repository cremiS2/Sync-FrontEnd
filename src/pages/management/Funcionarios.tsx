      ...
      <main className={`transition-all duration-300 pt-16 ml-0 ${
        ...
        <div className="p-4 md:p-6 min-h-screen animate-fadeInUp" style={{ background: 'var(--bg-gradient)' }}>
            ...
            <div className={`rounded-xl sm:rounded-2xl shadow-sm border-2 p-3 sm:p-5 md:p-6 hover:shadow-lg transition-all duration-300 ${
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 text-[10px] sm:text-xs text-green-600 font-medium">
                    <span>{Math.round((activeEmployees/totalEmployees)*100)}% equipe</span>
                  </div>
                </div>
                <div className={`w-8 h-8 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${
                  isDarkMode ? "bg-green-900/30" : "bg-green-100"
                }`}>
                  <FaUserTie className={`text-sm sm:text-lg md:text-xl ${
            ...
            <div className={`rounded-xl sm:rounded-2xl shadow-sm border-2 p-3 sm:p-5 md:p-6 hover:shadow-lg transition-all duration-300 ${
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 text-[10px] sm:text-xs text-green-600 font-medium">
                    <span>+2.1% vs anterior</span>
                  </div>
                </div>
                <div className={`w-8 h-8 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${
                  isDarkMode ? "bg-yellow-900/30" : "bg-yellow-100"
                }`}>
                  <FaChartLine className={`text-sm sm:text-lg md:text-xl ${
          ...
          <ResponsiveGrid
            ...
            renderItem={(employee) => (
              <div
                className={`rounded-2xl shadow-sm border p-8 hover:shadow-lg transition-all duration-300 cursor-pointer h-full overflow-hidden flex flex-col ${
                  isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
                }`}
              ...
              >
                  ...
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className={`text-xl font-bold mb-2 ${
                        isDarkMode ? "text-white" : "text-gray-900"
                      }`} title={employee.name}>{employee.name.length > 18 ? employee.name.substring(0, 18) + '...' : employee.name}</h3>
                      <p className={`text-sm ${
                        isDarkMode ? "text-gray-400" : "text-gray-600"
                      }`} title={employee.role || '-'}>{(employee.role || '-').length > 20 ? (employee.role || '-').substring(0, 20) + '...' : (employee.role || '-')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 mb-6 flex-grow">
                  <div className={`flex items-center gap-3 text-sm ${
                    ...
                    <span title={employee.department || '-'}>{(employee.department || '-').length > 20 ? (employee.department || '-').substring(0, 20) + '...' : (employee.department || '-')}</span>
                  </div>
                  <div className={`flex items-center gap-3 text-sm ${
                    isDarkMode ? "text-gray-300" : "text-gray-700"
                  }`}>
                    ...
                    <span title={employee.shift || '-'}>{employee.shift || '-'}</span>
                  </div>
                  <div className={`flex items-center gap-3 text-sm ${
                    isDarkMode ? "text-gray-300" : "text-gray-700"
                  }`}>
            ...
            <>
              <DialogTitle className="bg-gradient-to-r from-blue-600 to-blue-800 text-white" sx={{ overflow: 'hidden' }}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    />
                    <div className="min-w-0 flex-1">
                      <h2 className="text-xl font-semibold truncate" title={selectedEmployee.name}>{selectedEmployee.name}</h2>
                      <p className="text-sm opacity-90 truncate" title={selectedEmployee.role}>{selectedEmployee.role}</p>
                    </div>
                  </div>
                  <button
              ...
              <DialogContent className="p-8" style={{ backgroundColor: isDarkMode ? '#1e293b' : '#ffffff', overflow: 'auto' }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-5">
                    <div className={`p-4 rounded-lg border overflow-hidden ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-blue-50 border-blue-200'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <FaUserTie className={`flex-shrink-0 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                        <span className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Cargo</span>
                      </div>
                      <p className={`text-sm font-medium break-words ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`} title={selectedEmployee.role}>{selectedEmployee.role}</p>
                    </div>

                    <div className={`p-4 rounded-lg border overflow-hidden ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-blue-50 border-blue-200'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <FaBuilding className={`flex-shrink-0 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                        <span className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Departamento</span>
                      </div>
                      <p className={`text-sm font-medium break-words ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`} title={selectedEmployee.department}>{selectedEmployee.department}</p>
                    </div>

                    <div className={`p-4 rounded-lg border overflow-hidden ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-blue-50 border-blue-200'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <FaClock className={`flex-shrink-0 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                        <span className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Turno</span>
                      </div>
                      <p className={`text-sm font-medium break-words ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`} title={selectedEmployee.shift}>{selectedEmployee.shift}</p>
                    </div>
                  ...
                  <div className="space-y-5">
                    {/* Máquinas associadas */}
                    <h4 className={`text-lg font-bold ${isDarkMode ? 'text-blue-400' : 'text-blue-700'}`}>Máquinas Operadas</h4>
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        const maquinas = machines.filter((maq: any) => selectedEmployee.machineIds.includes(maq.id));
                        if (maquinas.length === 0) return <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-800'}`}>Nenhuma máquina associada.</span>;
