    ...
    <div className="w-screen min-h-screen" style={{ background: 'var(--bg-gradient)' }}>
      ...
      <main className={`transition-all duration-300 pt-16 ml-0 ${
        ...
        <div className="p-4 md:p-6 min-h-screen animate-fadeInUp" style={{ background: 'var(--bg-gradient)' }}>
          ...
          {/* Filtros */}
          <div className={`rounded-xl sm:rounded-2xl shadow-sm border p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 md:mb-8 hover:shadow-lg transition-all duration-300 ${
            isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
          }`}>
            <h3 className={`text-base sm:text-lg md:text-xl font-bold mb-3 sm:mb-4 md:mb-6 ${
              isDarkMode ? "text-white" : "text-gray-900"
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 md:gap-6 mb-4 sm:mb-6 md:mb-8 items-start"
            ...
            <div className={`rounded-xl sm:rounded-2xl shadow-sm border-2 p-3 sm:p-5 md:p-6 hover:shadow-lg transition-all duration-300 ${
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <p className={`text-[10px] sm:text-xs font-semibold mb-1 sm:mb-2 uppercase tracking-wide truncate ${
                    isDarkMode ? "text-gray-400" : "text-gray-600"
                  }`}>Departamentos</p>
            ...
            <div className={`rounded-xl sm:rounded-2xl shadow-sm border-2 p-3 sm:p-5 md:p-6 hover:shadow-lg transition-all duration-300 ${
              ...
              <div className="flex items-start justify-between">
                ...
                <div className={`w-8 h-8 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${
                  ...
                  <FaDollarSign className={`text-sm sm:text-lg md:text-xl ${
            ...
            <div className={`rounded-xl sm:rounded-2xl shadow-sm border-2 p-3 sm:p-5 md:p-6 hover:shadow-lg transition-all duration-300 ${
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 text-[10px] sm:text-xs text-red-600 font-medium">
                    <span>{inactiveDepartments} dept.</span>
                  </div>
                </div>
                <div className={`w-8 h-8 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0 ${
                  isDarkMode ? "bg-red-900/30" : "bg-red-100"
                }`}>
                  <FaTools className={`text-sm sm:text-lg md:text-xl ${
          ...
          <ResponsiveGrid
            ...
            renderItem={(department) => (
              <div className={`rounded-xl sm:rounded-2xl shadow-sm border p-3 sm:p-4 md:p-6 lg:p-8 hover:shadow-lg transition-all duration-300 h-full overflow-hidden flex flex-col ${
                isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
              }`}>
                <div className="mb-2 sm:mb-4 md:mb-6">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0 w-full overflow-hidden">
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-16 lg:h-16 rounded-lg sm:rounded-xl md:rounded-2xl flex items-center justify-center flex-shrink-0 ${
                      isDarkMode ? "bg-blue-900/30" : "bg-blue-100"
                    }`}>
                    ...
                    <div className="min-w-0 flex-1 overflow-hidden">
                      <h3 className={`text-xs sm:text-sm md:text-base lg:text-xl font-bold mb-0.5 sm:mb-1 md:mb-2 truncate ${
                        isDarkMode ? "text-white" : "text-gray-900"
                      }`} title={department.name}>{department.name}</h3>
                      <p className={`text-[10px] sm:text-xs md:text-sm truncate ${
                        isDarkMode ? "text-gray-400" : "text-gray-600"
                      }`} title={department.location}>{department.location}</p>
                    </div>
                  </div>
                </div>

                <p className={`text-[10px] sm:text-xs md:text-sm mb-2 sm:mb-4 md:mb-6 line-clamp-2 ${
                  isDarkMode ? "text-gray-300" : "text-gray-700"
                }`} title={department.description}>
                  {department.description}
                </p>

                <div className="grid grid-cols-2 gap-1.5 sm:gap-2 md:gap-4 mb-2 sm:mb-4 md:mb-6 flex-grow">
                  <div className={`text-center p-1.5 sm:p-2 md:p-4 rounded-lg sm:rounded-xl md:rounded-2xl ${
                    isDarkMode ? "bg-gray-700/50" : "bg-gray-50"
                  }`}>
                    <p className={`text-sm sm:text-lg md:text-xl lg:text-2xl font-bold mb-0.5 ${
                      isDarkMode ? "text-white" : "text-gray-900"
                ...
                <div className="mt-auto">
                  <Button
                    variant="outlined"
                    startIcon={<FaEye className="text-[10px] sm:text-xs md:text-sm" />}
                    onClick={async () => {
      ...
      <Dialog
        ...
        >
          {selectedDepartment && (
            <>
              <DialogTitle className="bg-gradient-to-r from-blue-600 to-blue-800 text-white" sx={{ overflow: 'hidden' }}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <h2 className="text-xl font-semibold truncate" title={selectedDepartment.name}>{selectedDepartment.name}</h2>
                      <p className="text-sm opacity-90 truncate" title={selectedDepartment.location}>{selectedDepartment.location}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setSelectedDepartment(null)}
                    aria-label="Fechar"
                    className="w-10 h-10 bg-white/30 rounded-lg flex items-center justify-center text-white hover:bg-white hover:text-red-600 transition-all duration-200 font-bold text-xl flex-shrink-0"
                  >
                    <FaTimes />
                  </button>
                </div>
              ...
              <DialogContent className="p-8" style={{ backgroundColor: isDarkMode ? '#1e293b' : '#ffffff', overflow: 'auto' }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                  <div className="space-y-5">
                    <div className={`p-4 rounded-lg border overflow-hidden ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-blue-50 border-blue-200'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <FaUserTie className={`flex-shrink-0 ${isDarkMode ? 'text-blue-400' : 'text-[var(--primary-dark)]'}`} />
                        <span className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Gerente</span>
                      </div>
                      <p className={`text-sm font-medium truncate ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>João Silva</p>
                    </div>

                    <div className={`p-4 rounded-lg border overflow-hidden ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-blue-50 border-blue-200'}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <FaMapMarkerAlt className={`flex-shrink-0 ${isDarkMode ? 'text-blue-400' : 'text-[var(--primary-dark)]'}`} />
                        <span className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Localização</span>
                      </div>
                      <p className={`text-sm font-medium break-words ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`} title={selectedDepartment.location}>{selectedDepartment.location}</p>
                    </div>
                  ...
                  <div>
                    <div className={`p-4 rounded-lg border mb-4 ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-blue-50 border-blue-200'}`}>
                      ...
                      <p className={`text-sm font-medium break-words ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`} title={selectedDepartment.description}>{selectedDepartment.description}</p>
                    </div>
