...
const Departamentos: React.FC = () => {
  ...
  const handleSubmitDepartment = async (data: Record<string, any>) => {
    try {
      ...
      const errorMessage = error?.response?.data?.message || 
                          ...
                          'Erro ao salvar departamento. Verifique os dados e tente novamente.';
  ...
  const handleSubmitSector = async (data: Record<string, any>) => {
    try {
      ...
      notification.error(
        error?.response?.data?.menssagem || error.message || `Erro ao ${sectorFormMode === 'create' ? 'criar' : 'editar'} setor`,
        'Erro'
      );
    } finally {
      setLoading(false);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                  <div className="space-y-5">
                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-bold text-gray-900">Gerente</span>
                      </div>
                      <p className="text-sm text-gray-800 font-medium">João Silva</p>
                    </div>

                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center gap-2 mb-2">
                        <FaMapMarkerAlt className="text-[var(--primary-dark)]" />
                        <span className="text-sm font-bold text-gray-900">Localização</span>
                      </div>
                      <p className="text-sm text-gray-800 font-medium">{selectedDepartment.location}</p>
                    </div>

                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center gap-2 mb-2">
                        <FaUsers className="text-[var(--primary-dark)]" />
                        <span className="text-sm font-bold text-gray-900">Funcionários</span>
                      </div>
                      <p className="text-sm text-gray-800 font-medium">{departmentEmployeeCounts[selectedDepartment.id] || 0} colaboradores</p>
                    </div>

                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center gap-2 mb-2">
                        <FaDollarSign className="text-[var(--primary-dark)]" />
                        <span className="text-sm font-bold text-gray-900">Orçamento</span>
                      </div>
                      <p className="text-sm text-gray-800 font-medium">R$ {selectedDepartment.budget.toLocaleString()}</p>
                    </div>

                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center gap-2 mb-2">
                        <FaCalendarAlt className="text-[var(--primary-dark)]" />
                        <span className="text-sm font-bold text-gray-900">Criado em</span>
                      </div>
                      <p className="text-sm text-gray-800 font-medium">{new Date(selectedDepartment.createdAt).toLocaleDateString('pt-BR')}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200 mb-4">
                      ...
                      <p className="text-sm text-gray-800 font-medium">{selectedDepartment.description}</p>
                    </div>

                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-bold text-[var(--primary-dark)] text-lg">Setores</h4>
                        <Button
                          variant="contained"
                          startIcon={<FaPlus />}
