    ...
    <div className="w-screen min-h-screen" style={{ background: 'var(--bg-gradient)' }}>
      ...
      <main className={`transition-all duration-300 pt-16 ml-0 ${sidebarCollapsed ? 'md:ml-16' : 'md:ml-64'}`}>
        ...
        <div className="p-4 md:p-6 min-h-screen animate-fadeInUp" style={{ background: 'var(--bg-gradient)' }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 md:gap-6 mb-4 sm:mb-6 md:mb-8 items-start"
            ...
            {[
              { label: 'Total Itens', value: totalItems, icon: FaBoxes, color: '#3b82f6' },
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredItems.map((item) => (
              <div key={item.id}
                className={`rounded-2xl shadow-sm border p-8 hover:shadow-lg transition-all duration-300 cursor-pointer overflow-hidden h-full flex flex-col ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
                <div className="flex items-start justify-between mb-6 gap-2">
                  <div className="flex-1 min-w-0">
                    <h3 className={`text-xl font-bold mb-2 ${isDarkMode ? "text-white" : "text-gray-900"}`} title={item.nome}>{item.nome.length > 18 ? item.nome.substring(0, 18) + '...' : item.nome}</h3>
                    <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>{item.categoria}</p>
                  </div>
                  <Chip label={getStatusText(item.status)} color={getStatusColor(item.status) as any} size="small" icon={getStatusIcon(item.status)} sx={{ flexShrink: 0 }} />
                </div>
                <div className="space-y-4 mb-6 flex-grow">
                  <div className={`flex items-center justify-between text-sm gap-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <span className="flex-shrink-0">Código:</span>
                    <span className="font-semibold" title={item.codigo}>{item.codigo.length > 15 ? item.codigo.substring(0, 15) + '...' : item.codigo}</span>
                  </div>
                  <div className={`flex items-center justify-between text-sm gap-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <span className="flex-shrink-0">Quantidade:</span>
                    <span className="font-bold text-lg">{item.quantidade} {item.unidade}</span>
                  </div>
                  <div className={`flex items-center justify-between text-sm gap-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <span className="flex-shrink-0">Fornecedor:</span>
                    <span className="font-semibold" title={item.fornecedor}>{item.fornecedor.length > 15 ? item.fornecedor.substring(0, 15) + '...' : item.fornecedor}</span>
                  </div>
                  <div className={`flex items-center gap-3 text-sm ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
                    <FaIndustry className={`text-lg flex-shrink-0 ${isDarkMode ? "text-blue-400" : "text-blue-600"}`} />
                    <span title={item.localizacao}>{item.localizacao.length > 20 ? item.localizacao.substring(0, 20) + '...' : item.localizacao}</span>
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <span className={`text-sm flex-shrink-0 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>Entrada:</span>
                    <span className="text-sm font-semibold">{new Date(item.dataEntrada).toLocaleDateString('pt-BR')}</span>
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <span className={`text-sm flex-shrink-0 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>Preço Unit.:</span>
          ...
          <DialogContent className="p-6" style={{ backgroundColor: isDarkMode ? '#1e293b' : '#ffffff' }}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="p-4 rounded-lg overflow-hidden" style={{ backgroundColor: isDarkMode ? '#0f172a' : '#f1f5f9' }}>
                  <h4 className="font-bold mb-2" style={{ color: isDarkMode ? '#f1f5f9' : '#1e293b' }}>Código</h4>
                  <p className="text-sm break-all" style={{ color: isDarkMode ? '#94a3b8' : '#475569' }}>{selectedItemForDetails.codigo}</p>
                </div>
                <div className="p-4 rounded-lg overflow-hidden" style={{ backgroundColor: isDarkMode ? '#0f172a' : '#f1f5f9' }}>
                  <h4 className="font-bold mb-2" style={{ color: isDarkMode ? '#f1f5f9' : '#1e293b' }}>Quantidade</h4>
                  <p className="text-lg font-bold break-all" style={{ color: isDarkMode ? '#f1f5f9' : '#1e293b' }}>{selectedItemForDetails.quantidade} {selectedItemForDetails.unidade}</p>
                </div>
                <div className="p-4 rounded-lg overflow-hidden" style={{ backgroundColor: isDarkMode ? '#0f172a' : '#f1f5f9' }}>
                  <h4 className="font-bold mb-2" style={{ color: isDarkMode ? '#f1f5f9' : '#1e293b' }}>Fornecedor</h4>
                  <p className="text-sm break-all" style={{ color: isDarkMode ? '#94a3b8' : '#475569' }}>{selectedItemForDetails.fornecedor}</p>
                </div>
                <div className="p-4 rounded-lg overflow-hidden" style={{ backgroundColor: isDarkMode ? '#0f172a' : '#f1f5f9' }}>
                  <h4 className="font-bold mb-2" style={{ color: isDarkMode ? '#f1f5f9' : '#1e293b' }}>Localização</h4>
                  <p className="text-sm break-all" style={{ color: isDarkMode ? '#94a3b8' : '#475569' }}>{selectedItemForDetails.localizacao}</p>
                </div>
              </div>
              <div className="space-y-4">
                <div className="p-4 rounded-lg overflow-hidden" style={{ backgroundColor: isDarkMode ? '#0f172a' : '#f1f5f9' }}>
            ...
            {selectedItemForDetails.descricao && (
              <div className="mt-6 p-4 rounded-lg overflow-hidden" style={{ backgroundColor: isDarkMode ? '#0f172a' : '#f1f5f9' }}>
                <h4 className="font-bold mb-2" style={{ color: isDarkMode ? '#f1f5f9' : '#1e293b' }}>Descrição</h4>
                <p className="text-sm break-words" style={{ color: isDarkMode ? '#94a3b8' : '#475569' }}>{selectedItemForDetails.descricao}</p>
              </div>
            )}
          </DialogContent>
      ...
      <CUDModal
        ...
        fields={[
          ...
          { name: 'nome', label: 'Nome do Item', type: 'text', required: true, maxLength: 100 },
          { 
            name: 'categoria', 
            label: 'Categoria', 
            type: 'select', 
            ...
            options: [
              ...
              { value: 'Pneumáticos', label: 'Pneumáticos' },
              { value: 'Consumíveis', label: 'Consumíveis' },
              { value: 'Outros', label: 'Outros' }
            ]
          },
          { name: 'quantidade', label: 'Quantidade', type: 'number', required: true, minValue: 0, maxValue: 999999 },
          { name: 'unidade', label: 'Unidade de Medida', type: 'unit', required: true },
          { name: 'precoUnitario', label: 'Preço Unitário', type: 'currency', required: true, minValue: 0, maxValue: 9999999.99 },
          { name: 'fornecedor', label: 'Fornecedor', type: 'text', required: true, maxLength: 100 },
          { name: 'dataEntrada', label: 'Data de Entrada', type: 'date', required: true, maxDate: new Date().toISOString().split('T')[0] },
          { name: 'dataValidade', label: 'Data de Validade', type: 'date', required: true, minDate: new Date().toISOString().split('T')[0] },
          { name: 'localizacao', label: 'Localização', type: 'text', required: true, maxLength: 100, placeholder: 'Ex: Prateleira A1, Galpão 2' },
          { 
            name: 'status', 
            label: 'Status', 
            type: 'select', 
          ...
          { name: 'descricao', label: 'Descrição', type: 'textarea', required: false, maxLength: 500, rows: 4 },
        ] as FormFieldConfig[]}
