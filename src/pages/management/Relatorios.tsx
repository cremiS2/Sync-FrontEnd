...
import type { Machine, Employee, Sector, Stock } from '../../types';
...
const Relatorios: React.FC = () => {
  ...
  // Dados de qualidade baseados no estoque
  ...
  const outStock = stockItems.filter(item => item.status === 'OUT_OF_STOCK').length;
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-gradient)' }}>
      ...
      <main className={`transition-all duration-300 ${sidebarCollapsed ? 'ml-20' : 'ml-64'} pt-16 px-8`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <FormControl fullWidth>
                ...
                >
                  ...
                  {sectors.map(sector => (
                    <MenuItem key={sector.id} value={sector.id.toString()}>{sector.name}</MenuItem>
                  ))}
                </Select>
              </FormControl>
              ...
              <div className={`rounded-2xl shadow-sm border p-8 ${
                ...
                <div className="space-y-6">
                  {getReportData().slice(0, 4).map((item, index) => (
                    <div key={index} className={`flex justify-between items-center p-6 rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 ${
                      isDarkMode ? "bg-gray-700 hover:bg-gray-600" : "bg-gray-50 hover:bg-gray-100"
                    }`}>
                      <div>
                        <p className={`text-lg font-semibold mb-2 ${
                          isDarkMode ? "text-white" : "text-gray-900"
                        }`}>{getItemDisplayName(item)}</p>
                        <p className={`text-sm ${
                          isDarkMode ? "text-gray-400" : "text-gray-600"
