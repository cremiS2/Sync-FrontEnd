...
const Atribuicoes: React.FC = () => {
  ...
  const handleSubmitAllocation = async (data: Record<string, any>) => {
    ...
    // Salvar o título localmente (a API não suporta esse campo ainda)
    const allocationTitle = data.title || 'Atribuição';
    
    const dto: AllocatedEmployeeMachineDTO = {
      employee: parseInt(data.employee) || 0,
      machine: parseInt(data.machine) || 0
    };
    ...
    try {
      ...
      const errorMsg = err?.response?.data?.menssagem || err?.response?.data?.message || err.message;
      notification.error(
        `Erro ao salvar atribuição: ${errorMsg}`,
        'Erro'
      );
    }
  };
