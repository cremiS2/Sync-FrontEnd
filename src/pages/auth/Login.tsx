...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Senha length:', pass.length);
    ...
    try {
      ...
      console.log('[Login] Token:', localStorage.getItem('user_token') ? 'Presente' : 'Ausente');
      console.log('[Login] UserRole:', localStorage.getItem('userRole'));
      
      // Verificar se há uma URL salva para redirecionar
      ...
      <main className="flex-1 w-full flex items-center justify-center fade-in">
        <div className={styles.formBox}>
          ...
          <div className="mt-4 text-center fade-in-up" style={{ animationDelay: '450ms' }}>
            <span className="text-[var(--muted)]">Não tem uma conta? </span>
            <Link to="/cadastro-usuarios" className="text-[var(--primary)] hover:underline font-semibold transition-all duration-200">Cadastre-se aqui</Link>
          </div>
        </div>
      </main>
    </div>
