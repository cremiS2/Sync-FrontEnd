...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const operatingMachines = machines.filter((machine: any) => machine.status === 'OPERATING' || machine.status === 'ACTIVE').length;
      const maintenanceMachines = machines.filter((machine: any) => machine.status === 'MAINTENANCE').length;
      
      const totalDepartments = departments.length;
      const activeDepartments = departments.filter((dept: any) => dept.status === 'active').length;
      ...
      departments.slice(0, 1).forEach((dept: any) => {
        activities.push({
          ...
          type: 'department',
          title: dept.name,
          description: `Departamento ${dept.status === 'active' ? 'ativo' : 'inativo'} - Orçamento: R$ ${(dept.budget || 0).toLocaleString()}`,
          time: '1 hora atrás',
          status: dept.status === 'active' ? 'success' : 'warning'
        });
      });
  ...
  const chartData = {
    production: [
      { label: 'Jan', value: Math.round(stats.monthlyProduction * 0.8), color: '#3B82F6' },
      { label: 'Fev', value: Math.round(stats.monthlyProduction * 0.9), color: '#10B981' },
      { label: 'Mar', value: Math.round(stats.monthlyProduction * 0.7), color: '#F59E0B' },
      { label: 'Abr', value: Math.round(stats.monthlyProduction * 1.1), color: '#EF4444' },
      { label: 'Mai', value: Math.round(stats.monthlyProduction * 0.95), color: '#8B5CF6' },
      { label: 'Jun', value: stats.monthlyProduction, color: '#06B6D4' }
    ],
    departments: [
    ...
    <div className="w-screen min-h-screen" style={{ background: 'var(--bg-gradient)' }}>
      ...
      <main className={`transition-all duration-300 pt-16 ${sidebarCollapsed ? 'ml-16' : 'ml-64'
        ...
        <div className="p-6 min-h-screen" style={{ background: 'var(--bg-gradient)' }}>
          ...
          {/* Bottom Section - Activities and Actions */}
              ...
              <div className={`rounded-2xl shadow-sm border p-8 hover:shadow-lg transition-all duration-300 ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
                ...
                <div className="space-y-4">
                  <Button
                    ...
                    startIcon={<FaPlus />}
                    fullWidth
                    onClick={() => handleNavigation('/management/maquinas')}
                    sx={{
                      backgroundColor: 'var(--primary)',
                      color: 'white',
                      fontSize: '0.875rem',
                  ...
                  <Button
                    ...
                    startIcon={<FaUsers />}
                    fullWidth
                    onClick={() => handleNavigation('/management/funcionarios')}
                    sx={{
                      color: 'var(--primary)',
                      borderColor: 'var(--primary)',
                      fontSize: '0.875rem',
                  ...
                  <Button
                    ...
                    fullWidth
                    onClick={() => handleNavigation('/management/departamentos')}
                    sx={{
                      color: 'var(--primary)',
                      borderColor: 'var(--primary)',
                      fontSize: '0.875rem',
                  ...
                  <Button
                    ...
                    fullWidth
                    onClick={() => handleNavigation('/dashboard/relatorios')}
                    sx={{
                      color: 'var(--primary)',
                      borderColor: 'var(--primary)',
                      fontSize: '0.875rem',
              ...
              <div className={`rounded-2xl shadow-sm border p-8 hover:shadow-lg transition-all duration-300 ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <Chip label="Online" color="success" size="small" sx={{ fontSize: '0.75rem', height: '24px', borderRadius: '8px' }} />
                  </div>
                  <div className="flex items-center justify-between">
                    <span className={`text-sm font-medium ${isDarkMode ? "text-gray-400" : "text-gray-600"
                      }`}>Banco de Dados</span>
                    <Chip label="Conectado" color="success" size="small" sx={{ fontSize: '0.75rem', height: '24px', borderRadius: '8px' }} />
                  </div>
                  <div className="flex items-center justify-between">
                    <span className={`text-sm font-medium ${isDarkMode ? "text-gray-400" : "text-gray-600"
                      }`}>Backup</span>
                    <Chip label="Atualizado" color="success" size="small" sx={{ fontSize: '0.75rem', height: '24px', borderRadius: '8px' }} />
                  </div>
                </div>
              </div>
            </div>
