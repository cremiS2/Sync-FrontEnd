  ...
  const loadStatisticsData = async () => {
    try {
      ...
      const machineOEE: MachineOEE[] = machines.map((machine: any, index: number) => ({
        ...
        availability: Math.round(85 + Math.random() * 15), // 85-100%
        performance: Math.round(80 + Math.random() * 20), // 80-100%
        quality: Math.round(90 + Math.random() * 10), // 90-100%
        oee: Math.round((machine.oee || 0.8) * 100),
        downtime: Math.round(Math.random() * 5), // 0-5 horas
        production: Math.round((machine.throughput || 100) * (0.8 + Math.random() * 0.4)), // Variação da produção
